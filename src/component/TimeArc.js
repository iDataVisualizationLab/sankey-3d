...
// }
    renderSankey(opts = {}, linearScale = 0) {
        ...
        area .x(d => x(d[0]))
            .y0((d) => {
                return -this.yScale(d[1])/2;
            })
            .y1((d) => {
                return this.yScale(d[1])/2;
            });

        this.y.range([0, this.heightG()]).padding(this.padding);
        this.y.domain(nodes.map(d=>({y:d.y,name:d.name})).sort((a,b)=>a.y-b.y).map(d=>d.name));
        this.x.range([0, this.widthG()]);
        const pNodes = nodes.filter(d=>d.first);
        this.yScale.domain([0,d3.max(pNodes.map(d=>d3.max(d.drawData,e=>e[1])))]).range([0,this.heightG()/pNodes.length])
        nodes.forEach(d=>{
            d.x = this.x(d.time);
            d.y = this.y(d.name);
            d.width = this.yScale(d.value)
        });
...
// }
    ...
    linkPath(d) {
        let source = [this.x(d.source.time), this.y(d.source.name)];
        let target = [this.x(d.target.time), this.y(d.target.name)];
        ...
        const thick = this.yScale(d.value)/2;
    ...
    render() {
        ...
        const {margin, nodes, linksBySource, isForceDone, highlight, highlightJob,graph} = this.state;
                ...
                {isForceDone ? <g className={'content' + (highlight ? ' onhighlight' : '')}>
                        ...
                        <g className={'drawArea'}>
                            <g className={'nodes'}>
                                ...
                                {pNodes.map(d => <path key={d.name} className={".layerpath"} transform={`translate(${0},${d.y})`}
                                          fill={d.color||'unset'} stroke={d.color||'unset'}
                                          strokeWidth={1}
                                          d={d.drawData[0]?area([d.drawData[0],...d.drawData,d.drawData[d.drawData.length-1]]):''}/>)}
                            </g>

                            <g className={'links'}>
                                {
                                    links.filter(d=>d.arr).map((d,i)=><g key={d._id}>
                                        <linearGradient id={d._id} gradientUnits={"userSpaceOnUse"} x1={d.source.x} x2={d.target.x}>
                                            <stop offset={'0%'} stopColor={d.source.color||this.color(d.source)}/>
                                            <stop offset={'100%'} stopColor={d.target.color||this.color(d.target)}/>
                                        </linearGradient>
                                        {/*<path fill={`url(#${d._id})`} stroke={`url(#${d._id})`} strokeWidth={0.5} d={this.linkPath(d)}/>*/}
                                        <path fill={`white`} stroke={`white`} strokeWidth={0.5} d={this.linkPath(d)}/>
                                    </g>)
                                }
                                {/*{linksBySource.map(d => <NodeElement*/}
                                        {/*className={'outer_node element ' + d.values[0]._class + (d.hide ? ' hide' : '') + ((highlight && (highlight.list[d.key])) ? ' highlight' : '')}*/}
                                        {/*key={d.key}*/}
...
export function horizontalSource(d) {
    return [d.source.x, d.source.y-d.source.width/2];
}

export function horizontalTarget(d) {
    return [d.target.x, d.target.y+d.source.width/2];
}
